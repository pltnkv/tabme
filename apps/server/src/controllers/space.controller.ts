...
class SpaceController {
  async getUserSpaces(req: Request, res: Response): Promise<void> {
    try {
      ...
      logError(error, "Failed to get user spaces")
  ...
  async getSpaceById(req: Request, res: Response): Promise<void> {
    try {
      ...
      logError(error, `Failed to get space by ID: ${req.params.spaceId}`)
  ...
  async createSpace(req: Request, res: Response): Promise<void> {
    try {
      ...
      logError(error, "Failed to create space")
  ...
  async updateSpace(req: Request, res: Response): Promise<void> {
    try {
      ...
      logError(error, `Failed to update space: ${req.params.spaceId}`)
  ...
  async deleteSpace(req: Request, res: Response): Promise<void> {
    try {
      ...
      logError(error, `Failed to delete space: ${req.params.spaceId}`)
  ...
  async updateSpacePositions(req: Request, res: Response): Promise<void> {
    try {
      ...
      logError(error, "Failed to update space positions")
  ...
  async addUserToSpace(req: Request, res: Response): Promise<void> {
    try {
      ...
      logError(error, `Failed to add user to space: ${req.params.spaceId}`)
      if (error instanceof Error && error.message.includes("not found")) {
        res.status(404).json({ error: error.message })
      } else {
  ...
  async removeUserFromSpace(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!targetUserId) {
        res.status(400).json({ error: "targetUserId should be provided" })
        return
      }
      ...
      logError(error, `Failed to remove user from space: ${req.params.spaceId}`)
  ...
  async getSpaceUsers(req: Request, res: Response): Promise<void> {
    try {
      ...
      logError(error, `Failed to get space users: ${req.params.spaceId}`)

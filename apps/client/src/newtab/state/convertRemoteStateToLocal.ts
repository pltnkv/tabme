...
type RemoteWidget = components["schemas"]["Widget"];
...
interface RemoteState {
  user?: RemoteUser & { name?: string | null; settings: RemoteUserSettings | null };
  spaces?: RemoteSpace[];
...
export function convertRemoteStateToLocal(remoteState: RemoteState): Partial<IStoredAppState> {
  ...
  const spaces: ISpace[] = sortByPosition(remoteState.spaces.map((remoteSpace): ISpace => {
    const folders: IFolder[] = sortByPosition((remoteSpace.folders ?? []).map((remoteFolder): IFolder => {
      const items: IFolderItem[] = sortByPosition((remoteFolder.items ?? []).map((remoteItem): IFolderItem => {
        return {
          id: genUniqLocalId(),
          remoteId: remoteItem.id as any, // ID mismatch: server string, client number
  ...
  const settings = remoteState.user?.settings
  ...
  const localState: Partial<IStoredAppState> = {
    ...
    // hasHiddenObjects: false // default
  }
